...
cat(paste("📊 Response: Retrieved", length(authorities), "authorities\n\n"))
...
cat(paste("\n🌍 Your account has access to", length(unique_auths), "unique authorities:\n"))
...
if (!is.null(first_authority)) {
  cat(paste("📌 We'll use '", first_authority, "' as our example authority for the following demonstrations\n", sep = ""))
} else {
  cat("⚠️ No authorities found for this account\n")
}
```
...
cat(paste("\n📈 Found", length(unique_surveys), "surveys for '", first_authority, "':\n", sep = ""))
cat(paste(rep("=", 60), collapse = ""), "\n")
...
cat(paste("\n🐟 Found", length(unique_species), "unique species organized into", 
...
cat(paste("\n📋 Found", nrow(reports_df), "available reports:\n"))
...
if ("data" %in% names(response_data)) {
  ...
  cat("⚠️ No 'data' field found in response\n")
}
```
...
cat(paste("📁 Data saved to:", output_filename, "\n"))
...
cat(paste("📈 Approximate records:", format(line_count - 1, big.mark = ","), "(excluding header)\n"))
cat(paste("📂 File location:", normalizePath(output_filename), "\n"))
```
...
cat("🔍 Demonstrating filtered data export...\n\n")
...
cat(paste("📋 Using report: '", selected_report_name, "'\n", sep = ""))
cat(paste("🌍 Filtering by authority: '", first_authority, "'\n", sep = ""))
...
if (nrow(species_df) > 0) {
  ...
  cat(paste("🐟 Filtering by species: '", species_name, "'\n", sep = ""))
} else {
  # Use example species ID
  species_filter <- list("d791be4d-593f-42f0-9a97-8c8d2181b9fe")
  cat("🐟 Using example species filter\n")
}

cat("\n🔧 API Call: POST /FieldSurveys/LdsStatistics/ExportDataAsCsv (with filters)\n")
...
cat("\n✅ CSV export successful!\n")
cat(paste("📁 Data saved to:", filtered_filename, "\n"))

# Show some statistics
filtered_line_count <- length(readLines(filtered_filename, warn = FALSE))
cat(paste("📈 Approximate records:", format(filtered_line_count - 1, big.mark = ","), "(excluding header)\n"))
cat(paste("📂 File location:", normalizePath(filtered_filename), "\n"))
```
